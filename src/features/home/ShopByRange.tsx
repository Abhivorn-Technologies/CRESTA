...
export const ShopByRange = React.memo(function ShopByRange() {
  ...
  useEffect(() => {
    ...
    async function fetchCategories() {
      try {
        const res = await fetch(`/api/categories?_cb=${Date.now()}`, { cache: "no-store" });
        if (res.ok) {
          ...
          <div className="flex flex-1 overflow-hidden relative">
            ...
            >
              {categories.map((category, i) => (
                ...
                >
                  {/* Image Circle */}
                  <div className="relative flex h-[160px] sm:h-[180px] w-full items-center justify-center transition-transform duration-500 group-hover:-translate-y-2">
                    <div className={`absolute inset-0 mx-auto w-[160px] sm:w-[180px] rounded-[2rem] bg-gradient-to-br ${category.color} opacity-10 blur-xl group-hover:opacity-30 transition-opacity`} />
                    <div className="relative flex h-[140px] w-[140px] sm:h-[160px] sm:w-[160px] items-center justify-center overflow-hidden rounded-full bg-transparent">
                      <Image src={category.image} alt={category.name} fill priority={i < itemsToShow} unoptimized={true} className="object-contain drop-shadow-lg"  sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-1 text-center mt-2 sm:mt-0">
                    ...
                    <span className="mt-2 text-[10px] sm:text-[12px] font-bold text-[#e6127d] bg-[#fdeef6] px-4 py-1.5 rounded-full group-hover:bg-[#e6127d] group-hover:text-white transition-colors duration-300">
                      {category.count}
                    </span>
                  </div>
                </div>
