    ...
    <div className="w-full flex flex-col md:flex-row gap-8 items-start">
      ...
      <div className="w-full md:w-[280px] shrink-0 flex flex-col gap-5 md:sticky md:top-24">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="px-4 md:px-6 py-4 md:py-5 border-b border-gray-100">
            <h3 className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 md:mb-4">Order Status</h3>
            <div className="flex flex-row md:flex-col overflow-x-auto gap-3.5 pb-2 md:pb-0 hide-scrollbar whitespace-nowrap">
              {ORDER_STATUS_FILTERS.map(status => (
                <label key={status} className="flex items-center gap-2.5 md:gap-3.5 cursor-pointer group shrink-0">
                  ...
                  />
                  <div className={`w-4.5 h-4.5 rounded shadow-sm border flex items-center justify-center transition-all ${statusFilters.includes(status) ? 'bg-[#101b4d] border-[#101b4d]' : 'bg-gray-50 border-gray-200 group-hover:border-[#101b4d]'}`}>
                    {statusFilters.includes(status) && <div className="w-2 h-2 bg-white rounded-[1px]" />}
                  </div>
                  <span className={`text-[15px] font-medium transition-colors ${statusFilters.includes(status) ? 'text-[#101b4d]' : 'text-gray-600 group-hover:text-[#101b4d]'}`}>{status}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="px-4 md:px-6 py-4 md:py-5">
            <h3 className="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 md:mb-4">Order Time</h3>
            <div className="flex flex-row md:flex-col overflow-x-auto gap-3.5 pb-2 md:pb-0 hide-scrollbar whitespace-nowrap">
              {ORDER_TIME_FILTERS.map(time => (
                <label key={time} className="flex items-center gap-2.5 md:gap-3.5 cursor-pointer group shrink-0">
                  ...
                  />
                  <div className={`w-4.5 h-4.5 rounded shadow-sm border flex items-center justify-center transition-all ${timeFilters.includes(time) ? 'bg-[#101b4d] border-[#101b4d]' : 'bg-gray-50 border-gray-200 group-hover:border-[#101b4d]'}`}>
                    {timeFilters.includes(time) && <div className="w-2 h-2 bg-white rounded-[1px]" />}
                  </div>
                  <span className={`text-[15px] font-medium transition-colors ${timeFilters.includes(time) ? 'text-[#101b4d]' : 'text-gray-600 group-hover:text-[#101b4d]'}`}>{time}</span>
                </label>
              ))}
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col gap-6 w-full mt-2 md:mt-7">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex items-stretch focus-within:ring-[3px] focus-within:ring-[#101b4d]/10 focus-within:border-[#101b4d] transition-all">
          ...
          <button className="bg-[#101b4d] text-white px-5 md:px-12 text-[13px] md:text-[15px] font-bold hover:bg-[#0a1133] transition-colors shrink-0">
            Search
          </button>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[500px]">
          {isLoading ? (
            ...
            <div className="flex-1 flex flex-col gap-4 items-center justify-center py-20 text-center">
