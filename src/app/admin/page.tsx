...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      setLoading(true);
      try {
        const response = await fetch(`/api/admin/dashboard?timeframe=${timeframe}&_cb=${Date.now()}`, { cache: "no-store" });
        if (response.ok) {
  ...
  if (!data) {
    return (
      <div className="flex-1 w-full flex items-center justify-center text-red-500 font-medium">
