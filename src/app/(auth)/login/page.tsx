...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Route based on actual returned role from DB
      if (data.user.role === "admin") router.push("/admin");
      else if (data.user.role === "seller") router.push("/seller/dashboard");
      else if (data.user.role === "delivery_partner") router.push("/driver");
      else router.push("/");
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4" autoComplete="off">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="flex justify-end mt-1">
