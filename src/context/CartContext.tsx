...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // Custom setter for buyNowItem to sync with sessionStorage
  const setBuyNowItem = (item: CartItem | null) => {
    setBuyNowItemState(item);
    if (typeof window !== "undefined") {
      if (item) {
        sessionStorage.setItem("cresta_buynow", JSON.stringify(item));
      } else {
        sessionStorage.removeItem("cresta_buynow");
      }
    }
  };

  // Initialize buyNowItem from sessionStorage
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = sessionStorage.getItem("cresta_buynow");
      if (saved) {
        try {
  ...
  return (
    <CartContext.Provider value={{ cart, addToCart, removeFromCart, updateQuantity, clearCart, cartTotal, cartCount, buyNowItem, setBuyNowItem }}>
      {children}
